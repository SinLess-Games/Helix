...
SinLess Games development team follows the latest security best practices when
developing software, and automates security testing throughout development
lifecycle whenever possible.

Security is integrated into all phases of SinLess Games product development lifecycle,
including:
...
Details about the SinLess Games software application architecture and
...
(a) SinLess Games software engineering and product development is required to follow
...
third-party components and dependencies included in SinLess Games software products.
...
Software development at SinLess Games follows a release strategy that provides
...
(HubSpot) kicks off unit and functional tests which pass before changes are
...
SinLess Games practices continuous delivery of code into production
...
SinLess Games’s multiple lower environments (dev, test) provide an ecosystem of
...
All SinLess Games `git` repositories have a company standard configuration from a
...
().  Continuous integration (HubSpot) kicks off unit and functional
...
SinLess Games practices continuous delivery of code into production through multiple
...
SinLess Games development/engineering team uses  for
...
All SinLess Games software must be developed to include the following general
...
reported and tracked following SinLess Games Vulnerability Management process
...
SinLess Games Security Team in collaboration with development team performs full
...
Documentation on the SinLess Games [Engineering Wiki]()
may include additional security specifications as well as the security design
and implementation details of the SinLess Games Platform and its
...
SinLess Games external software application that is customer facing with
...
- their access activities are logs (Accounting/Auditing) according to the
  SinLess Games auditing standards.
...
SinLess Games security and development team implemented a process to
...
functionality is embedded into the `securityScan()` HubSpot pipeline step.
...
-   The Lint and GitLeaks Platform - static and dynamic code analysis. Together with
    ...
    SinLess Games HubSpot pipeline library via the `securityScan()` step
...
SinLess Games uses a third party platform to maintain a public
...
SinLess Games's platform and its public facing resources.
...
SinLess Games requires all outsourced software development to follow the
...
associate" of ours.  It is required by HIPAA for SinLess Games and AWS to
...
details on SinLess Games's HIPAA compliance.
