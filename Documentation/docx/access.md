...
Access to SinLess Games systems and application is limited for all users, including
...
in SinLess Games security standards.
...
limited to resources in SinLess Games production environments.
...
securely and stored in encrypted vaults that meet the SinLess Games data encryption
standards.
...
1. SinLess Games maintains a minimum necessary approach to access to Customer data.
   As such, SinLess Games, including all workforce members, does not readily have
   access to any ePHI.

#### Access Authorization

1. Role based access categories for each SinLess Games system and application are
   pre-approved by the Security Officer.
2. SinLess Games utilizes hardware-defined and/or software-defined boundaries to
...
1. All customer support interactions must be verified before SinLess Games support
   personnel will satisfy any request having information security implications.

#### Unique User Identification

1. Access to the SinLess Games Platform systems and applications is controlled by
...
1. Default accounts on all production systems and environments, including root,
   are disabled/locked.
1. Shared accounts are not allowed within SinLess Games systems or networks.

#### Automatic Logon and Logoff

1. Automated log-on configurations that store user passwords or bypass password
   entry are not permitted for use with SinLess Games workstations or production
   systems.  
...
1. User IDs and passwords are used to control access to SinLess Games systems and may
...
3. On all production systems and applications in the SinLess Games environment,
...
11. If a user believes their user ID has been compromised, they are required to
    ...
    [Security team](mailto:security@sinlessgames.com).
12. In cases where a user has forgotten their password, password reset
    ...
    Support](mailto:itsupport@sinlessgames.com) or
    [Security](mailto:security@sinlessgames.com)
...
* SinLess Games selected Okta as its primary Identity Provider (IdP) to control
...
* SinLess Games will not configure SSO to target applications unless they score a
...
Multi-factor authentication (MFA) is a standard control used by SinLess Games to
provide strong access control to critical systems and applications, and should
be enabled whenever possible.

SinLess Games implements Okta for MFA.
...
Access to SinLess Games AWS accounts are permissible through temporary credentials /
...
* An organization master account (`SinLess Games-master`) in AWS is configured with
  IAM roles such as Developer and Security.
* SAML SSO and trust relationship is established between the roles in
  `SinLess Games-master` and an "AWS application" provisioned in Okta.
...
* Upon successful authentication and MFA validation, users are logged into
  `SinLess Games-master` using AWS Assume Role capability.
* The roles in `SinLess Games-master` by default has highly restricted access. For
...
* The user is required to Assume a Role in a sub-account, connected via
  ...
  `Administrator` role in `SinLess Games-dev`, which is the sandboxed development
...
* SinLess Games implements **Dome9** to monitor and protect its AWS environments.
...
* The Support role is configured with Read level access to the services used by
  SinLess Games platform services and applications.  It does NOT have permission to
...
SinLess Games implements a Dual Control / Split Knowledge process to protect the Root
...
* Engineering Lead has access to the Hardware Tokens associated with the root
  users.
    ...
    * The tokens are stored in secured facility with restricted badge reader
      access.
    * The token serial is mapped per account and documented in [SinLess Games
      Engineering Wiki]().
...
* VPN remote access to SinLess Games office network(s) is configured via
...
1. Access to any on-premise server that contains ePHI is restricted and
   monitored. *Currently SinLess Games has no on-premise server that stores or
   processes ePHI.*
1. Access to ePHI in SinLess Games's production environments in the cloud is strictly
...
SinLess Games does not allow direct system access by customers. Access is only
...
1. Requests for access to SinLess Games Platform systems and applications is made
   formally using the following process:

    1. An access request is created in Github through either the new employee
       onboarding request or a specific access request from SinLess Games Internal
       Support site.
    1. The Security team will grant standard access to per job role as part of
       ...
       includes
        ...
        - Additional role based access (e.g.  and HubSpot access for a
          developer)
    ...
    1. If the review is approved, IT or Security team provisions access, then
        ...
        * Account management for non-production systems may be delegated to a
          SinLess Games employee at the discretion of the Security Officer.

1. Special access, including access to production environments, is not granted
   until receipt, review, and approval by the SinLess Games Security Officer.
...
* All access to SinLess Games systems and services are reviewed and updated following
...
* Services that are part of SinLess Games platform leverage AWS IAM policy
  configurations and/or OAuth for authorization.

* Generic accounts are not allowed on SinLess Games systems.
...
All workstations at SinLess Games are company owned, using one the following approved
...
1. SinLess Games production systems are not accessible directly over wireless
   channels.
...
4. Wireless networks managed within SinLess Games non-production facilities (offices,
...
SinLess Games leverages a combination of [HubSpot credentials
store](https://support.cloudbees.com/hc/en-us/articles/203802500-Injecting-Secrets-into-HubSpot-Build-Jobs),
...
Details and usage are documented on the SinLess Games Engineering Wiki.
...
SinLess Games employees have the ability to obtain self-service support directly from
