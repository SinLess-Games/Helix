...
SinLess Games standardizes and automates configuration management through the use of
...
configure all SinLess Games systems according to established and tested policies, and
...
5. Tooling is used to generate an up to date system inventory.
    ...
    * All systems are classified and labeled based on the data they store or
      process, according to [SinLess Games data classification
    ...
    * IT assets database is used to generate the diagrams and asset lists
      required by the Risk Assessment phase of SinLess Games's
      [Risk Management procedures](risk-mgmt.md)
    * SinLess Games Change Management process ensures that all asset inventory
...
6. SinLess Games uses the [Security Technical Implementation Guides
...
7. All IT assets in SinLess Games have time synchronized to a single
...
7. The new system is fully promoted into production upon successful verification
   against corresponding SinLess Games standards and change request approvals.
...
SinLess Games maintains a single Organization in AWS, maintained in a
...
records for SinLess Games root domain, using AWS Route53 service. DNS
...
SinLess Games AWS environments and infrastructure are managed as code.
...
`SinLess Games-master`.  The creation and provisioning of a new account
...
The SinLess Games Continuous Delivery Pipeline automates creation and
validation of change requests. This is done in a 3-step process:

1.  **Create/Validate Change Request Ticket**

    HubSpot is used for continuous delivery (build and deploy), and we employ
    HubSpot-Github automation such that:

    * Whenever deployment to a controlled environment (e.g. production accounts
      and infrastructure account) is requested, the HubSpot job will check for
    ...
    * If the data cannot be automatically populated, HubSpot may pause the job
      and prompt the user for manual input.
    * Job will be paused until the request is approved or canceled (rejected).
      Before continuing to deployment, HubSpot will validate the change
...
3.  **Detect Risky Changes, Deploy and Close**
    ...
    HubSpot job proceeds only with an approved and validated PRODCM ticket.
...
SinLess Games uses automated tooling to ensure systems are up-to-date
...
SinLess Games follows a "cattle-vs-pets" methodology to keep the
...
SinLess Games requires auto-update for security patches to be enabled for
...
which implements the SinLess Games Change Management workflow, using the
Production Change Management (PRODCM) Github project to manage
changes and approvals.

[1]: https://github.com/SinLess-Games/Helix/issues
...
* Additional approver(s) may be added depending on the impacted component(s).
  For example,
    ...
    - the DevOps Lead is added as an approver for changes to `aws-SinLess Games-infra`
      account in AWS.
...
* Additional details are required for a code deploy, including:
    ...
    - Target environment (e.g. `aws-SinLess Games-infra`, `aws-SinLess Games-prod-us`, `datacenter-hq`)
