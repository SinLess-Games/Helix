...
In order to preserve the integrity of data that SinLess Games stores, processes, or
transmits for Customers, SinLess Games implements strong intrusion detection tools
...
3. All systems are to only be used for SinLess Games business needs.
...
* SinLess Games leverages AWS GuardDuty and Cisco Meraki for network security of its on-premise
  environments.
...
* SinLess Games IT manager is responsible for configuring the firewall and IDS/IPS
  rules and review the configuration as least quarterly.

#### Intrusion Detection in AWS Cloud Environments

SinLess Games implemented a real-time threat detection solution by
...
SinLess Games is an active member of the [National Health Information Sharing and
Analysis Center (NH-ISAC)](https://nhisac.org/).

SinLess Games Security team is subscribed to receive threat alerts from NH-ISAC.
