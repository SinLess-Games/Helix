...
SinLess Games implements policies and procedures to maintain compliance and integrity
of data. The Security Officer and Privacy Officer are responsible for
maintaining policies and procedures and assuring all SinLess Games workforce members,
...
(a) SinLess Games policies must be developed and maintained to meet all
...
(c) All policy changes must be approved by SinLess Games Security Officer. Additionally,

  * Major changes may require approval by SinLess Games CEO or designee;
...
(e) Policy exceptions are handled on a case-by-case basis.
  ...
  * All policy exceptions must be approved by both SinLess Games Security Officer and COO.
...
Each SinLess Games policy document contains a version and optionally a
...
* Policy may be referenced by its statement number, such as `§2.1(a)`, in
  internal/external communications as well as in other SinLess Games policies or
...
1. All policies are stored and up to date to maintain SinLess Games compliance with
   HIPAA,
...
3. SinLess Games employees may request changes to policies using the following
   process:

    1. The SinLess Games employee initiates a policy change request by creating an
    ...
    5. If the policy change requires technical modifications to production
       ...
       SinLess Games's [change management process](ccm.md).
...
4. All policies are made accessible to all SinLess Games workforce members. The
   current master policies are published at
   [https://sinlessgames.com/security](https://sinlessgames.com/security).

    * Changes are automatically communicated to all SinLess Games team members
      through integrations between  and Slack that log changes
      to a predefined SinLess Games Slack Channel.
...
5. All policies, and associated documentation, are retained for 7 years from the
   ...
   later
     ...
     1. Version history of all SinLess Games policies is done via .
...
6. The policies and information security policies are reviewed and audited
   annually, or after significant changes occur to SinLess Games's
   organizational environment, by the security committee members. Issues that
   come up as part of this process are reviewed by SinLess Games
