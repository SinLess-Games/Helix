...
This policy outlines the requirements and controls/procedures SinLess Games has
...
Data backup is an important part of the day-to-day operations of SinLess Games. To
protect the confidentiality, integrity, and availability of sensitive and critical data, both for
SinLess Games and SinLess Games Customers, complete backups are done daily to assure that
...
the [SinLess Games data classification model](data-mgmt.md#data-classification-model),
...
(d) Data must be backed up according to its level defined in SinLess Games data
classification.
...
regulatory and contractual requirements.  More specifically,
  ...
  * Data and records belonging to SinLess Games platform customer must be retained
    per SinLess Games product terms and conditions and/or specific contractual
    agreements.

(g) By default, all security documentation and audit trails are kept for a
minimum of seven years, unless otherwise specified by SinLess Games data
...
SinLess Games defines the following four classifications of data:
...
of retention, are defined according to the SinLess Games Data
Classifications.
...
SinLess Games customer, or as required by regulations, whichever is
...
The SinLess Games platform will automatically adjust the storage class for certain
types of data based on its usage pattern and age. This allows the SinLess Games
...
data. In most cases, the SinLess Games platform automatically adjusts the storage
...
SinLess Games performs regular full backups of all production data. We leverage S3
...
*   Unstructured business documents such as Word documents, Excel spreadsheets
    and PowerPoint presentations are stored on SinLess Games internal
    file share.
...
*SinLess Games currently does NOT use transient storage for any sensitive data.*
...
SinLess Games stores data in a secure production account in AWS, using a
...
SinLess Games performs automatic backup of all customer and system data to protect
...
Customers can also utilize the SinLess Games Application Programming Interface (API)
to extract and store their data elsewhere. Standard API usage fees will apply.

#### Source code

SinLess Games stores its source in git repositories hosted by .

Source code repositories are backed up to SinLess Games’s AWS S3 infrastructure
...
SinLess Games SharePoint team site. Examples of business files include, but are not
limited to:
...
Despite not being a requirement within HIPAA, SinLess Games understands and
appreciates the importance of health data retention. Acting as a
subcontractor/service provider, and at times a business associate, SinLess Games is
not directly responsible for health and medical records retention as set forth
by each state.

SinLess Games has created and implemented the following procedures to
make it easier for SinLess Games Customers to support data retention
laws.
...
SinLess Games is NOT a covered entity.  Should we become a covered entity in the
