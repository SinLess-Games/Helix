...
class Roles(commands.Cog):
    ...
    async def modrole(self, ctx, *, role: BetterRoles = None):
        ...
        if role.is_default():
            return await ctx.send(
                "You can't mod everyone (If you for some reason really want to do this, give it to a role and assign it to everyone)")
        async for guild in self.client.fetch_guilds():
            self.c.execute(f'''SELECT mod_roles FROM {guild.id}.config WHERE guild_id=?''', (ctx.guild.id,))
        roles_already = self.c.fetchone()[0]

        new_role = role.id

        self.c.execute(f'''UPDATE {ctx.guild.id}config SET mod_roles=? WHERE guild_id=?''', (new_role, ctx.guild.id))
        self.conn.commit()
        self.client.modroles[str(ctx.guild.id)] = new_role
    ...
    async def _clear(self, ctx):
        ...
        self.conn.commit()
        self.Client.modroles[str(ctx.guild.id)] = None
    ...
    async def add(self, ctx, role: BetterRoles = None):
        ...
        if not str(role.id) in automatic_roles:
            ...
            return await ctx.send(
                "That role is already automatically assigned, use `!autorole remove` to remove the role.")
