...
class AutoMod(commands.Cog):
    def __init__(self, client):
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!', database=f"discord")
    ...
    async def twitch(self, ctx, *, channel: discord.TextChannel = None):
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"discord")
        self.c = self.conn.cursor()
        self.c.execute("UPDATE servers SET twitch_channel='{}' WHERE id='{}'".format(channel.id, ctx.message.guild.id))
        self.conn.commit()
    ...
    async def log(self, ctx, *, channel: discord.TextChannel = None):
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"discord")
        self.c = self.conn.cursor()
        self.c.execute("UPDATE servers SET log_channel='{}' WHERE id='{}'".format(channel.id, ctx.message.guild.id))
        self.conn.commit()
    ...
    async def _bot(self, ctx, channel: discord.TextChannel = None):
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"discord")
        self.c = self.conn.cursor()
        self.c.execute("UPDATE servers SET bot_channel='{}' WHERE id='{}'".format(channel.id, ctx.message.guild.id))
        self.conn.commit()
    ...
    async def welcome(self, ctx, *, channel: discord.TextChannel = None):
        database = ctx.guild.id
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"{database}")
        self.c = self.conn.cursor()
        self.c.execute("INSERT INTO greetings (guild_id, greet_channel, greet_message, farewell_message, ban_message) VALUES ('{}', '{}', '{}', '{}', '{}')".format(str(ctx.guild.id), None, None, None, None))
        self.conn.commit()
        if channel is None:
            self.c.execute("UPDATE greetings SET greet_channel='{}' WHERE guild_id='{}'".format(None, ctx.guild.id))
            self.conn.commit()
            return await ctx.send("Welcome channel removed.")

        self.c.execute("UPDATE greetings SET greet_channel='{}' WHERE guild_id='{}'".format(channel.id, ctx.guild.id))
        self.conn.commit()
    ...
    async def greet(self, ctx, *, msg: str = None):
        self.c.execute("INSERT OR IGNORE INTO greetings (guild_id, greet_channel, greet_message, farewell_message, ban_message) VALUES ('{}', '{}', '{}', '{}', '{}')".format(ctx.guild.id, None, None, None, None))
        self.conn.commit()
        if msg is None:
            self.c.execute("UPDATE greetings SET greet_message='{}' WHERE guild_id='{}'".format(None, ctx.message.guild.id))
            self.conn.commit()
            return await ctx.send("Greeting message removed.")
        msg = clean_string(msg)
        self.c.execute("UPDATE greetings SET greet_message='{}' WHERE guild_id='{}'".format(msg, ctx.guild.id))
        self.conn.commit()
    ...
    async def _cfg(self, ctx):
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"{database}")
        self.c = self.conn.cursor()
        self.c.execute("SELECT * FROM logging WHERE server='{}'".format(ctx.guild.id))
        logging_table = self.c.fetchone()
        self.c.execute("SELECT * FROM config WHERE guild_id='{}'".format(ctx.guild.id))
        config = self.c.fetchone()
        self.c.execute("SELECT user_id, plonked FROM userconfig WHERE guild_id='{}'".format(ctx.guild.id))
        id_and_plonk = self.c.fetchall()
        print(id_and_plonk)
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"discord")
        self.c = self.conn.cursor()
        self.c.execute("SELECT log_channel, twitch_channel, prefix FROM servers WHERE id='{}'".format(ctx.guild.id))
        cfg = self.c.fetchone()
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"{database}")
        self.c = self.conn.cursor()
        self.c.execute("SELECT * FROM greetings WHERE guild_id='{}'".format(ctx.guild.id))
    ...
    async def _log(self, ctx, logging: str = None):
        ...
        if logging is None or logging.lower() == "help":
            return await ctx.send(
                f"Usage `!log <action>` where action is one of the following: avatar, edit, role, delete, ban, join, name, ignore or unignore")
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"{database}")
        self.c = self.conn.cursor()
        self.c.execute("SELECT * FROM logging WHERE server='{}'".format(ctx.message.guild.id))
        logging_config = self.c.fetchone()
        if logging_config is None:
            # This only happens if the bot somehow missed adding the guild
            self.c.execute("INSERT INTO logging (server, message_edit, message_deletion, role_changes, name_update, member_movement, avatar_changes, bans) VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')".format(ctx.message.guild.id, 1, 1, 1, 1, 1, 1, 1))
            self.conn.commit()
        ...
        if "avatar" in logging:
            ...
            return await ctx.send(
                f"That isn't a valid subcommand, try: avatar, edit, role, delete, ban, join, name, ignore or unignore")
        ...
        self.c.execute(f"UPDATE logging SET {update}='{value}' WHERE server='{ctx.message.guild.id}'")
        self.conn.commit()
    ...
    async def ignore(self, ctx, channel: discord.TextChannel = None):
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"{database}")
        self.c = self.conn.cursor()
        self.c.execute("SELECT ignored_channels FROM logging WHERE server='{}'".format(ctx.message.guild.id))
        ...
        self.c.execute("UPDATE logging SET ignored_channels='{}' WHERE server='{}'".format(new_channels, ctx.message.guild.id))
        self.conn.commit()
    ...
    async def unignore(self, ctx, channel: discord.TextChannel = None):
        ...
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"{database}")
        self.c = self.conn.cursor()
        self.c.execute("SELECT ignored_channels FROM logging WHERE server='{}'".format(str(ctx.message.guild.id)))
        ...
        self.c.execute("UPDATE logging SET ignored_channels='{}' WHERE server='{}'".format(new_channels, str(ctx.message.guild.id)))
        self.conn.commit()
    ...
    async def on_member_ban(self, guild, user):
        ...
        self.c = self.conn.cursor()
        self.c.execute(
            "SELECT log_channel FROM servers WHERE id='{}''".format(str(guild.id)))
        ...
        self.c.execute("SELECT * FROM logging WHERE server='{}'".format(str(guild.id)))
    ...
    async def on_member_unban(self, guild, user):
        ...
        predicate = self.c.execute(
            "SELECT * FROM logging WHERE server='{}'".format(str(guild.id)))
        predicate = self.c.fetchone()
        if predicate[7]:
    ...
    async def on_member_update(self, before, after):
        if before is None:
            return
        database = before.guild.id
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"{database}")
        self.c = self.conn.cursor()
        self.c.execute("SELECT log_channel FROM servers WHERE id='{}'".format(str(before.guild.id)))
        log_channel = self.c.fetchone()
        if log_channel is None or log_channel[0] is None:
            log_channel = None
        else:
            log_channel = self.client.get_channel(id=int(log_channel[0]))
        if before.display_name != after.display_name:
            self.c.execute("SELECT names FROM users WHERE (server='{}' AND id='{}')".format(before.guild.id, before.id))
            ...
            self.c.execute("UPDATE users SET names='{}' WHERE (id='{}' AND server='{}')".format(new_names, before.id, before.guild.id))
            self.conn.commit()
            ...
            # No codeblocks
            fmt = re.sub("([`])", r"\\\1", fmt)
            predicate = self.c.execute("SELECT * FROM logging WHERE server='{}'".format(before.guild.id))
            predicate = self.c.fetchone()
            if predicate[4] and log_channel is not None:
                ...
                async for entry in before.guild.audit_logs(limit=100, action=action):
                    ...
                    if entry.target == after:
                        if (before.display_name == entry.before.nick and (
                                after.display_name == entry.after.nick or entry.after.nick is None)) or (
                                entry.before.nick is None and after.display_name == entry.after.nick):
                            if entry.target != entry.user:
                                await msg.edit(content=fmt_edit.format(before, before.display_name, after.display_name,
                                                                       entry.user))
                                return
                            else:
                                await msg.edit(
                                    content=fmt_edit_self.format(before, before.display_name, after.display_name))
                                return

        elif before.roles != after.roles:
            roles = ','.join([str(x.id)
                              for x in after.roles if x.name != "@everyone"])
            self.c.execute("UPDATE users SET roles='{}' WHERE (id='{}' AND server='{}')".format(roles, before.id, before.guild.id))
            self.conn.commit()
            if len(before.roles) < len(after.roles):
                ...
                if predicate[3] and log_channel is not None:
                    ...
                    async for entry in before.guild.audit_logs(limit=100,
                        ...
                        if entry.target == after:
                            if entry.target != entry.user:
                                ...
                                return
                            await msg.edit(
                                content=":warning: **{}#{}** added the role **{}** to themselves.".format(before.name,
                ...
                if len(newrole) == 1:
                    ...
                    fmt = ":warning: **{}#{}** had the roles **{}** removed.".format(
                        before.name, before.discriminator, ', '.join(new_roles))
                predicate = self.c.execute(
                    "SELECT * FROM logging WHERE server='{}'".format(before.guild.id))
                predicate = self.c.fetchone()
                if predicate[3] and log_channel is not None:
                    msg = await log_channel.send(clean_string(fmt))
                    perms = before.guild.me.guild_permissions
                    ...
                    async for entry in before.guild.audit_logs(limit=100,
                        ...
                        if entry.target == after:
                            if entry.target != entry.user:
                                ...
                                return
                            await msg.edit(
                                content=":warning: **{}#{}** removed the role **{}** from themselves.".format(
                                    before.name, before.discriminator, newrole[0].name))
                            return
            ...
            predicate = self.c.execute("SELECT * FROM logging WHERE server='{}'".format(after.guild.id))
            predicate = self.c.fetchone()
            if predicate[6] and log_channel is not None:
                await log_channel.send(
                    fmt.format(after.name, after.discriminator, after.avatar_url.replace(".webp", ".png")))

    async def on_member_join(self, member):
        self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                            database=f"discord")
        self.c = self.conn.cursor()
        log_channel = self.c.execute("SELECT log_channel FROM servers WHERE id='{}'".format(member.guild.id))
        log_channel = log_channel.fetchone()
        if log_channel is not None and log_channel[0] is not None:
            log_channel = member.guild.get_channel(int(log_channel[0]))
        try:
            if member.guild.id == 241693350711787522:
            ...
            database = member.guild.id
            self.conn = mysql.connector.connect(host='192.168.86.41', user='admin', password='Shellshocker93!',
                                                database=f"{database}")
            self.c = self.conn.cursor()
            predicate = self.c.execute(
                "SELECT * FROM logging WHERE server='{}'".format(member.guild.id))
            predicate = self.c.fetchone()
            if predicate[5]:
        ...
        if rolestobeadded:
            try:
                ...
                await chn.send(
                    "Failed to give roles: {} to {} in {}".format(', '.join(rolestobeadded), member, member.guild.name))
        ...
        if predicate[5]:
            ...
            async for entry in member.guild.audit_logs(limit=100):
                ...
                if entry.target == member and (
                        entry.action == discord.AuditLogAction.kick or entry.action == discord.AuditLogAction.ban):
    ...
    async def on_message(self, message):
        ...
        if self.conn.is_connected():
            print(f'connected to {message.guild.id}')
        ...
        if message.attachments:
            ...
            if re.match(r"^(?:.*\.(mp4|webm|mkv|mov)$)$", filename):
                s = await message.channel.send(
                    "Discord doesn't embed direct uploads, would you like me to upload it to streamable? (y/n)")
                msg = await self.client.wait_for('message',
                                                 check=lambda
                                                     m: m.content.lower() == 'y' and m.author == message.author,
                                                 timeout=30.0)
                if msg:
                    await msg.delete()
                    ...
                    USER_AGENT = f'Helix BOT, {VERSION}, © 2021)'

                    async with self.bot.session.get(url="https://api.streamable.com/import?url={}".format(s_url),
                                                    headers={'User-Agent': USER_AGENT},
                                                    auth=aiohttp.BasicAuth(login="Helix@sinlessgames.com",
                                                                           password="D84GzYV3jx-c")) as r:
                        print(await r.text())
                        reply = await r.json()
                    await stream_msg.edit(
                        content="<https://streamable.com/{}> processing video...".format(reply['shortcode']))
                    embed = False
                    tries = 0
                    while not embed:
                        ...
                        if (tries % 5) == 0:
                            async with self.bot.session.get(
                                    "https://api.streamable.com/videos/{}".format(reply['shortcode'])) as r:
                                ree = await r.json()
                            if ree['status'] == 2:
                                embed = True
                            elif tries == 100:
    ...
    async def on_message_delete(self, message):
        ...
        poststring = clean_string(":x: `{1}` **{0}** deleted their message:  ```{2}``` in `{3}`".format(message.author,
                                                                                                        time.strftime(
                                                                                                            "%H:%M:%S"),
        ...
        if predicate[2]:
            ...
            async for entry in message.guild.audit_logs(limit=100, action=discord.AuditLogAction.message_delete):
                if entry.target == message.author:
                    if -10 < (entry.created_at - datetime.datetime.utcnow()).total_seconds() < 10:
                        await msg.edit(content=clean_string(
                            logstring.format(message.author, time.strftime("%H:%M:%S"), message.clean_content,
                                             message.channel, entry.user)))
                        return
    ...
    async def on_message_edit(self, before, after):
        ...
        if predicate[1]:
            await log_channel.send(
                fmt.format(time.strftime("%H:%M:%S"), before.author, before.clean_content, after.clean_content))
    ...
    async def hackban(self, ctx, userid, *, reason='Please write a reason!'):
        ...
        if not reason:
            return await ctx.send(f'Gimme a good reason to (•̆ꈊ•̆;;)▁▂▃▅▆▓▒░ {userid}')
