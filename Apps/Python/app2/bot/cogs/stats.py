...
LOGGING_CHANNEL = 881083005186240522
...
class Stats(commands.Cog):
    ...
    async def on_command(self, ctx):
        ...
        self.client.command_stats[command] += 1
        message = ctx.message
        destination = None
    ...
    async def on_socket_response(self, msg):
        self.client.socket_stats[msg.get('t')] += 1

    @commands.command(hidden=True)
    @commands.is_owner()
    async def commandstats(self, ctx, limit=20):
        counter = self.client.command_stats
    ...
    async def socketstats(self, ctx, limit=20):
        ...
        total = sum(self.client.socket_stats.values())
        cpm = total / minutes
        width = len(max(self.client.socket_stats, key=lambda x: len(str(x))))
        con = self.client.socket_stats.most_common(limit)
    ...
    async def uptime(self, ctx):
        ...
        em.set_author(name=self.client.user.name,
    ...
    async def activity(self, ctx, unit: str = "month"):
        ...
        # record = await self.client.loop.run_in_executor(None, thing)
        self.c.execute(f'''SELECT count(*) as c, author
                          FROM {ctx.guild.id}.messages
                          WHERE (server=? AND unix > ?)
                          GROUP BY author
                          ORDER BY c DESC;''',
                       (ctx.guild.id, diff))
        record = self.c.fetchall()
        e = discord.Embed(title=f"Activity for the last {unit}",
                          description=f"{sum(x[0] for x in record)} messages from {len(record)} chatters")
    ...
    async def char(self, ctx, unit: str = "day"):
        ...
        now = int(datetime.datetime.utcnow().timestamp())
        diff = now - time_seconds
        self.c.execute(f'''SELECT SUM(LENGTH(content)) as c, author, COUNT(*)
                          FROM {ctx.guild.id}.messages
                          WHERE (server=? AND unix > ?)
                          GROUP BY author
    ...
    async def about(self, ctx):
        embed = discord.Embed(description='HELIX 1.0')
        embed.title = 'About:'
        ...
        total_online = len(
            {m.id for m in self.client.get_all_members() if m.status is discord.Status.online})
        ...
        embed.add_field(name='Commands Run', value=sum(
            self.client.command_stats.values()))
    ...
    async def send_guild_stats(self, e, guild):
        ...
        bots = sum(m.client for m in guild.members)
        ...
        ch = self.client.get_channel(LOGGING_CHANNEL)
...
def setup(client):
    if not hasattr(client, 'command_stats'):
        client.command_stats = Counter()

    if not hasattr(client, 'socket_stats'):
