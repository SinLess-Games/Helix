...
class Context(commands.Context):
    ...
    async def prompt(self, message, *, timeout=60.0, delete_after=True, reacquire=True, author_id=None):
        ...
        if not self.channel.permissions_for(self.me).add_reactions:
            raise RuntimeError('client does not have Add Reactions permission.')
        ...
        try:
            await self.client.wait_for('raw_reaction_add', check=check, timeout=timeout)
        except asyncio.TimeoutError:
            confirm = None
    ...
    async def release(self):
        ...
        Otherwise, this is called automatically by the client.
        """
        # from source digging asyncpg source, releasing an already
        # released connection does nothing

        if self._db is not None:
            await self.client.pool.release(self._db)
            self._db = None
