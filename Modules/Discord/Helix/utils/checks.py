...
def check_if_it_is_Sinless_guild(ctx):
    """
    Works in DMs too - it will return False then.
    """
    if ctx.guild is None:
        return False
    elif ctx.guild.id != SinLess_guild_id:
        raise HelixGuildCheckFailure()
    else:
        return True


def Helix_bot_developer_only(ctx):
    """
    Check for commands only usable by Tortoise bot developers.
    """
    if ctx.author.id in Helix_developers:
        return True
    else:
        raise HelixBotDeveloperCheckFailure()
