...
class HelixAPI(BaseAPIClient):
    def __init__(self, *, loop: AbstractEventLoop):
        auth_header = {
            "Authorization": f"Token {os.getenv('API_ACCESS_TOKEN')}",
            "Content-Type": "application/json"
        }
        super().__init__("https://api.sinlessgames.com/private/", loop=loop, headers=auth_header)
    ...
    async def get_server_meta(self, guild_id: int = SinLess_guild_id) -> dict:
    ...
    async def edit_suggestion_message_id(self, new_id: int, guild_id: int = SinLess_guild_id) -> None:
