        ...
        async for guild in self.client.fetch_guilds():
            ...
            if not database_exists(Engine.url):
                ...
                Eng = create_engine(f'mysql+pymysql://{self.sql_user}:{self.sql_passwd}@{self.sql_host}/{guild.id}',
                                    echo=False)
                Engine = create_engine(
                    f'mysql+pymysql://{self.sql_user}:{self.sql_passwd}@{self.sql_host}/{self.sql_ddb}',
                    echo=False)
                ...
                with Session(Eng) as session:
                    stats = Stats()
                    _stats = session.query(Stats).filter_by(Date=datetime.today().strftime("%Y/%m/%d")).first()
                    ...
                    if not _stats:
                        ...
                        data = session.query(Stats).filter_by(Date=datetime.today().strftime("%Y/%m/%d")).first()
                        session.query(Stats).filter_by(Date=datetime.today().strftime("%Y/%m/%d")).update({
