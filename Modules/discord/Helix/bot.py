...
from Helix.utils.db_tools import Users, Stats, BlackList, Mutes, ServConfig, ServerList, Bans, Bots, Model
...
class Helix(commands.AutoShardedBot):
    ...
    async def on_shard_connect(self, shard_id):
        ...
        print(
            f"Shard {shard.id} | {self.user} Connected at {cT.hour}:{cT.minute}:{cT.second} / {cT.day}-{cT.month}-{cT.year} ")
    ...
    async def on_ready(self):
        ...
        async for guild in self.fetch_guilds():
            ...
            Model.__table__.create(bind=self.sql_engine, checkfirst=True)
