...
import com.sinlessgamesllc.config.KafkaProperties;
...
class HelixKafkaResourceIT {
    ...
    void setup() {
        ...
        HelixKafkaResource kafkaResource = new HelixKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/helix-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/helix-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
